if (compare_url == "OSR") {
    ...
    $(document).on("click", "#update-osr", function () {
        ...
        if ($("#status").val() == "RELEASED" && $("#released-to").val() == "") {
            ...
            if ($("#status").val() == "RELEASED") {
                ...
                $.notify({
                    title: '<strong>Checking User!</strong>',
                    message: "Please wait..."
                }, {
                    type: 'info',
                    z_index: 9999
                ...
                var ffID = $("#released-to").val();
                var password = $("#released-to-password").val();
                $.post(
                    ...
                    function (data) {
                        ...
                        if (data.done == "TRUE") {
                            var releasedTo = userDetailsSession;
                            update_osr(get_status,releasedTo);
                        }
                        else if (data.done == "FALSE") {
                            $("#released-to").addClass("is-invalid").removeClass("is-valid");
                            $.notify({
                                title: '<strong>INVALID USER TRY AGAIN!</strong>',
    ...
    function update_osr(get_status, releasedTo) {
